...
/**
 * Electric Furnace handler - Registry pattern
 *
 * This is a small handler class that implements the ElectricMachine interface.
 * All the actual implementation logic is in ElectricFurnaceMachine.java
 *
 * Functionality:
 * - Smelts items using energy from the network
 * - Tier 1 speed: 10 seconds per item (same as vanilla)
 * - Energy cost: 4J per smelt
 * - Must be connected to energy network within 6 blocks
 */
public class ElectricFurnaceHandler implements ElectricMachine {
    
    private static final int ENERGY_PER_SMELT = 50;
    private static final int PROCESSING_TIME_TICKS = 200; // 10 seconds (20 ticks/sec)
    ...
    public void handleInteraction(Block block, Player player, TechFactory plugin) {
        // Open Electric Furnace GUI
        ElectricFurnaceMachine.openInventory(block, player, plugin);
    }
    ...
    public void handleBreak(Block block, Player player, TechFactory plugin) {
        ...
        // Remove inventory
        ElectricFurnaceMachine.removeFurnace(location);
        
        // Drop Electric Furnace item
    ...
    public void dropInventoryItems(Location location, TechFactory plugin) {
        ElectricFurnaceMachine.dropInventoryItems(location, plugin);
    }
    
    @Override
    public RecipeItem getRecipeItem() {
        return new org.ThefryGuy.techFactory.recipes.workstations.electric.ElectricFurnace();
    }
}
